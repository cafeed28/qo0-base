...
-   number literals:
    ...
    1\. differentiate numbers use with _lowercase_ hex/binary/exponent/binary-exponent literals.
    ...
    2\. specify number data type with _UPPERCASE_ (except float, with _lowercase_) type literal.
...
-   string literals
    ...
    1\. wrap string that frequently contain code escapes with _UPPERCASE_ raw string literal.
...
1. preferred to be in _lowercase_, except when extra attention is required.
...
1. function name must be written in _PascalCase_.
...
1. variable name must be written in _camelCase_ and prefixed with hungarian notation of data type, if data type isn't specified nor based on STL/WinAPI data type, then _lowerCamelCase_ is used.
...
1. the name must be written in _UPPER_SNAKE_CASE_.
...
3. arguments must be written in _UPPER_SNAKE_CASE_.
